...
4. 用旧版可信工具生成与执行计划（以下命令由 Agent 操作）：
   ...
   最低维护起点为 **v1.0.0**（2026-09-13 起变更）；更早版本不在保证范围，保留其档案并说明需单独迁移评估。
...
对外发布版是**单一技能目录**，没有安装形态选择。升级步骤：

1. 记录当前版本与私人档案基线（条目数、revision）；
2. 完整备份旧核心与私人档案；
3. 用新版本目录**替换旧核心**，私人档案目录保持不动；
4. 运行核心校验与 `doctor`，核对档案基线未变；
5. 做一次**续写读回**：新增一条记录后能查到，确认读写正常。

若改用仓库内的受控更新器 `release_update.py`（`plan-install` → `install`），因候选相对旧版新增文件，需要提供
`--reviewed-manifest` 迁移审查收据（`emit-receipt-template` 生成骨架）；**直接替换目录不需要该收据**。
两种方式都不改变私人档案，也不重建空档案。