...
对外发布版是**单一技能目录**：解压后把 `review-evolution` 放进客户端技能目录即可使用；**没有可选的附加模块，也没有任何安装形态选择**。
需要 Python 3.10+；缺少运行时时先说明原因、征得同意后再按官方渠道安装。
升级：先备份旧核心，再用新版本目录替换；私人档案不随之改变，也不重建空档案。详细步骤见 [更新协议](update-brief.md)。
...
3. 两条复原路径，**任选其一**：
   - **有同版本原包**：用原包里的同名文件复原，然后重新 `verify` 到官方清单摘要；
   - **手边只有新版本候选包**（v1.3.1+）：在升级命令上加 **`--repair-host-rewrite`**，
     更新器会用**候选包中的同名官方文件**覆盖这些被改写文件，其余照常增量更新，结束后
     仍做整目录核验。它在计划里单列 `repair` 段（列出哪些文件与清单不符、装的是哪个摘要），
     属于**显式同意**；不加该参数时行为与以前完全一致（直接拒绝）。
     ```bash
     python -B scripts/release_update.py plan-install <候选目录> --target <技能目录> \
            --expected-version 1.3.1 --repair-host-rewrite
     python -B scripts/release_update.py install <候选目录> --target <技能目录> \
            --expected-version 1.3.1 --plan-id <上一步的 plan_id> --repair-host-rewrite
     ```
4. **不要修改 `CORE.json` 去迁就**被改写的文件——那等于自己伪造清单，之后所有校验都失去意义。
5. 复原后重跑 `plan-install`（上一步已带 `--repair-host-rewrite` 则直接完成升级）。

> 注意：`--repair-host-rewrite` 只影响**与自身清单不符**的那些文件，且要求候选包里存在同名文件；
> 它不会改变“新增加组件需要审查收据”等其他门禁，也不允许跳过整目录核验。
